...
function setSubmitBtn() {
    ELEMENTS.form.addEventListener('submit', async (event) => {
        ...
        if (response.status == "ok") {
            ...
            new notifications.NotificationMsg(() => {
                const specNameElement = document.getElementById('input-specName') as HTMLInputElement;
...
async function loadEditSpec(): Promise<boolean> {
    ...
    STATE.editID = await loading.loadSpec(specName, ELEMENTS.form, ELEMENTS.clientSelect);
